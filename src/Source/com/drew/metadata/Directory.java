...
// VARIOUS METHODS
	...
	/**
	 * Gets a value indicating whether the directory is empty, meaning it contains
	 * no errors and no tag values.
	 */
...
//        }
	...
	public void setObjectArray(int tagType, @NotNull Object array) {
		// for now, we don't do anything special -- this method might be a candidate for
		// removal once the dust settles
...
// TAG GETTERS
	...
	/**
	 * Returns the specified tag's value as an int, if possible. Every attempt to
	 * represent the tag's value as an int is taken. Here is a list of the action
	 * taken depending upon the tag's original type:
	 * <ul>
	 ...
	 * <li>String - Attempt to parse string as an int. If this fails, convert the
	 * char[] to an int (using shifts and OR).
	 ...
	 * </ul>
	 *
	 * @throws MetadataException if no value exists for tagType or if it cannot be
	 *                           converted to an int.
	 */
	public int getInt(int tagType) throws MetadataException {
		...
		if (o == null)
			throw new MetadataException(
					"Tag '" + getTagName(tagType) + "' has not been set -- check using containsTag() first");
		throw new MetadataException(
				"Tag '" + tagType + "' cannot be converted to int.  It is of type '" + o.getClass() + "'.");
	}

	/**
	 * Returns the specified tag's value as an Integer, if possible. Every attempt
	 * to represent the tag's value as an Integer is taken. Here is a list of the
	 ...
	 * </ul>
	 *
	 * If the value is not found or cannot be converted to int, <code>null</code> is
	 * returned.
	 */
	@Nullable
	 ...
	 * where the tag is set as StringValue[], String[], StringValue, String, int[],
	 * byte[] or Rational[].
	 *
	 * @param tagType the tag identifier
	 * @return the tag's value as an array of Strings. If the value is unset or
	 *         cannot be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	/**
	 * Gets the specified tag's value as a StringValue array, if possible. Only
	 * succeeds if the tag is set as StringValue[], or StringValue.
	 *
	 * @param tagType the tag identifier
	 * @return the tag's value as an array of StringValues. If the value is unset or
	 *         cannot be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	public double getDouble(int tagType) throws MetadataException {
		...
		if (o == null)
			throw new MetadataException(
					"Tag '" + getTagName(tagType) + "' has not been set -- check using containsTag() first");
		throw new MetadataException(
				"Tag '" + tagType + "' cannot be converted to a double.  It is of type '" + o.getClass() + "'.");
	}

	/**
	 * Returns the specified tag's value as a Double. If the tag is not set or
	 * cannot be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	public float getFloat(int tagType) throws MetadataException {
		...
		if (o == null)
			throw new MetadataException(
					"Tag '" + getTagName(tagType) + "' has not been set -- check using containsTag() first");
		throw new MetadataException(
				"Tag '" + tagType + "' cannot be converted to a float.  It is of type '" + o.getClass() + "'.");
	}

	/**
	 * Returns the specified tag's value as a float. If the tag is not set or cannot
	 * be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	public long getLong(int tagType) throws MetadataException {
		...
		if (o == null)
			throw new MetadataException(
					"Tag '" + getTagName(tagType) + "' has not been set -- check using containsTag() first");
		throw new MetadataException(
				"Tag '" + tagType + "' cannot be converted to a long.  It is of type '" + o.getClass() + "'.");
	}

	/**
	 * Returns the specified tag's value as a long. If the tag is not set or cannot
	 * be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	public boolean getBoolean(int tagType) throws MetadataException {
		...
		if (o == null)
			throw new MetadataException(
					"Tag '" + getTagName(tagType) + "' has not been set -- check using containsTag() first");
		throw new MetadataException(
				"Tag '" + tagType + "' cannot be converted to a boolean.  It is of type '" + o.getClass() + "'.");
	}

	/**
	 * Returns the specified tag's value as a boolean. If the tag is not set or
	 * cannot be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	/**
	 * Returns the specified tag's value as a java.util.Date. If the value is unset
	 * or cannot be converted, <code>null</code> is returned.
	 * <p>
	 * If the underlying value is a {@link String}, then attempts will be made to
	 * parse the string as though it is in the GMT {@link TimeZone}. If the
	 * {@link TimeZone} is known, call the overload that accepts one as an argument.
	 */
	@Nullable
	 ...
	 * <p>
	 * If the underlying value is a {@link String}, then attempts will be made to
	 * parse the string as though it is in the {@link TimeZone} represented by the
	 * {@code timeZone} parameter (if it is non-null). Note that this parameter is
	 * only considered if the underlying value is a string and it has no time zone
	 * information, otherwise it has no effect.
	 */
	@Nullable
	 ...
	 * {@code timeZone} parameter (if it is non-null). Note that this parameter is
	 * only considered if the underlying value is a string and it has no time zone
	 * information, otherwise it has no effect. In addition, the {@code subsecond}
	 * parameter, which specifies the number of digits after the decimal point in
	 * the seconds, is set to the returned Date. This parameter is only considered
	 * if the underlying value is a string and is has no subsecond information,
	...
	/**
	 * Returns the specified tag's value as a Rational. If the value is unset or
	 * cannot be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	/**
	 * Returns the specified tag's value as an array of Rational. If the value is
	 * unset or cannot be converted, <code>null</code> is returned.
	 */
	@Nullable
	...
	/**
	 * Returns the specified tag's value as a String. This value is the 'raw' value.
	 * A more presentable decoding of this value may be obtained from the
	 * corresponding Descriptor.
	 *
	 * @return the String representation of the tag's value, or <code>null</code> if
	...
	public String getString(int tagType) {
		...
		// Note that several cameras leave trailing spaces (Olympus, Nikon) but this
		// library is intended to show
		// the actual data within the file. It is not inconceivable that whitespace may
		// be significant here, so we
		// do not trim. Also, if support is added for writing data back to files, this
		// may cause issues.
...
// OTHER METHODS
	...
	public String toString() {
		return String.format("%s Directory (%d %s)", getName(), _tagMap.size(), _tagMap.size() == 1 ? "tag" : "tags");
	}
}
