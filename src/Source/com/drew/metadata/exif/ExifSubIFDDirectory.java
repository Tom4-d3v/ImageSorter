 ...
 */
package Source.com.drew.metadata.exif;
...
public class ExifSubIFDDirectory extends ExifDirectoryBase {
	...
	/**
	 * Parses the date/time tag, the subsecond tag and the time offset tag to obtain
	 * a single Date object with milliseconds representing the date and time when
	 * this image was modified. If the time offset tag does not exist, attempts will
	 * be made to parse the values as though it is in the GMT {@link TimeZone}.
	 *
	 * @return A Date object representing when this image was modified, if possible,
	 *         otherwise null
	 */
	@Nullable
	 ...
	 * a single Date object with milliseconds representing the date and time when
	 * this image was modified. If the time offset tag does not exist, attempts will
	 * be made to parse the values as though it is in the {@link TimeZone}
	 * represented by the {@code timeZone} parameter (if it is non-null).
	 *
	 ...
	 * Parses the date/time tag, the subsecond tag and the time offset tag to obtain
	 * a single Date object with milliseconds representing the date and time when
	 * this image was captured. If the time offset tag does not exist, attempts will
	 * be made to parse the values as though it is in the GMT {@link TimeZone}.
	 *
	 * @return A Date object representing when this image was captured, if possible,
	 *         otherwise null
	 */
	@Nullable
	 ...
	 * Parses the date/time tag, the subsecond tag and the time offset tag to obtain
	 * a single Date object with milliseconds representing the date and time when
	 * this image was digitized. If the time offset tag does not exist, attempts
	 * will be made to parse the values as though it is in the GMT {@link TimeZone}.
	 *
	 * @return A Date object representing when this image was digitized, if
	 ...
	 * a single Date object with milliseconds representing the date and time when
	 * this image was digitized. If the time offset tag does not exist, attempts
	 * will be made to parse the values as though it is in the {@link TimeZone}
	 * represented by the {@code timeZone} parameter (if it is non-null).
	 *
	 * @param timeZone the time zone to use
	 * @return A Date object representing when this image was digitized, if
	...
	@Nullable
	public TimeZone getTimeZone(int tagType) {
