 ...
 */
package Source.com.drew.imaging;

import java.io.IOException;
import java.io.InputStream;

import Source.com.drew.imaging.mp3.MpegAudioTypeChecker;
import Source.com.drew.imaging.quicktime.QuickTimeTypeChecker;
import Source.com.drew.imaging.riff.RiffTypeChecker;
import Source.com.drew.lang.ByteTrie;
...
public class FileTypeDetector {
	...
	static {
		_fixedCheckers = new TypeChecker[] { new QuickTimeTypeChecker(), new RiffTypeChecker(),
		...
		_root.addPath(FileType.Png, new byte[] { (byte) 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00,
		...
		// NOTE this doesn't work for NEF as it incorrectly flags many other TIFF files
		// as being NEF
//        _root.addPath(FileType.Nef, "MM".getBytes(), new byte[]{0x00, 0x2a, 0x00, 0x00, 0x00, (byte)0x08, 0x00});
		...
		_root.addPath(FileType.Asf, new byte[] { 0x30, 0x26, (byte) 0xB2, 0x75, (byte) 0x8E, 0x66, (byte) 0xCF, 0x11,
				(byte) 0xA6, (byte) 0xD9, 0x00, (byte) 0xAA, 0x00, 0x62, (byte) 0xCE, 0x6C });
		_root.addPath(FileType.Cfbf, new byte[] { (byte) 0xD0, (byte) 0xCF, 0x11, (byte) 0xE0, (byte) 0xA1, (byte) 0xB1,
				0x1A, (byte) 0xE1, 0x00 });
		_root.addPath(FileType.Flv, new byte[] { 0x46, 0x4C, 0x56 });
		_root.addPath(FileType.Indd, new byte[] { 0x06, 0x06, (byte) 0xED, (byte) 0xF5, (byte) 0xD8, 0x1D, 0x46,
				(byte) 0xE5, (byte) 0xBD, 0x31, (byte) 0xEF, (byte) 0xE7, (byte) 0xFE, 0x74, (byte) 0xB7, 0x1D });
		_root.addPath(FileType.Mxf,
				new byte[] { 0x06, 0x0e, 0x2b, 0x34, 0x02, 0x05, 0x01, 0x01, 0x0d, 0x01, 0x02, 0x01, 0x01, 0x02 }); // has
																													// offset?
		_root.addPath(FileType.Qxp, new byte[] { 0x00, 0x00, 0x49, 0x49, 0x58, 0x50, 0x52, 0x33 }); // "..IIXPR3"
																									// (little-endian -
																									// intel)
		_root.addPath(FileType.Qxp, new byte[] { 0x00, 0x00, 0x4D, 0x4D, 0x58, 0x50, 0x52, 0x33 }); // "..MMXPR3"
																									// (big-endian -
																									// motorola)
		...
		_root.addPath(FileType.Sit, new byte[] { 0x53, 0x74, 0x75, 0x66, 0x66, 0x49, 0x74, 0x20, 0x28, 0x63, 0x29, 0x31,
				0x39, 0x39, 0x37, 0x2D }); // StuffIt (c)1997-
	 ...
	 * <p>
	 * The input stream must support mark and reset, in order to return the stream
